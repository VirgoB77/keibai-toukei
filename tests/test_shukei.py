...
class 子の名前は2通りある(unittest.TestCase):
    ...
    def test_まとまりをまたいだ重なりも止める(self):
        """**同じ子が2つの親に属すと、どちらの足し算も正しくない。**

        まとまりの中だけを見る検査では通ってしまう
        （`落札` は接頭辞を持たないので、形では見分けられない）。
        2026-09-19 に、この入力を渡すテストが1本も無く、
        検査を外しても全部通ることが分かった。
        """
        with self.assertRaises(ValueError):
    ...
    def test_子がほかのまとまりの親と同じ名前なら止める(self):
        """親は出さない升。子の名前と重なると、
        **「出す升」と「出さない升」が同じ名前**になる。
        """
        with self.assertRaises(ValueError):
            Families(("結果", "落札", "不調"), ("不調", "甲", "乙"))
