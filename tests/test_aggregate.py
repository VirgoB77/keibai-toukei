...
class 黙って捨てない(unittest.TestCase):
    ...
    正本4節③の住所と同じ形。「空なら『つながらなかった』と分かるだけで済む」。
    """
    ...
    **消えた物件を全部「取下げ」と数えると、繰り越し待ちが混ざって
    ...
    def test_3つの述語は重なりうる(self):
        ...
        **重なる行を1つも渡していなかった**ので、いつでも通っていた。
        同じファイルの `test_not_countedの4つは行方から作る` が
...
class 足したときに数が合うこと(unittest.TestCase):
    ...
    正本の等式は「升の合計 ＋ not_counted の4つ ＝ 一覧で見た行の数」。
    **このサイトではそのままでは閉じない。** 1つの行が2つ以上の升に入り、
    ...
    def test_普通の一生を食い違いにしない(self):
        ...
        （正本 9節「壊れたものを探す見張りが、正しいものを壊れていると言う」）。
        """
    ...
    def test_not_countedの4つは行方から作る(self):
        """**数え方を2つ持たない**（正本 9節）。
...
class 見張りと出力語がずれない(unittest.TestCase):
    ...
    毎回鳴って誰も見なくなる（正本 9節「壊れたものを探す見張りが、正しいものを壊れていると言う」）。
    """
