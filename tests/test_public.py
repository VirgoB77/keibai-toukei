...
class 実数が出せた升の割合(unittest.TestCase):
    """正本 3.2「判定は『伏せ字の割合』ではなく『実数が出せた升の割合』で見る」（2026-09-17）。
...
class 余裕を測ってある(unittest.TestCase):
    """正本 3.2「足してから測るのではなく、足す前に測って、余裕が足りなければ層の作りを変える」。
...
class 出さない理由は居住の可能性(unittest.TestCase):
    """**出さない理由は「住所だから」ではなく「人が住んでいる可能性があるから」**（正本 3.1）。

    正本 3.1 は住所の扱いに例外を置いている。行政等が事業所・営業施設・届出事業の
    所在地として公式に公開しており、その所在地自体がサービスの対象なら、
    題材ごとに公開条件を確かめたうえで地図に出すことを検討できる。

    **競売・公売はこの例外に当たらない。** 入札中で、その家に人が
    住んでいることがあるから。`records = 0` は変わらないが、
    **理由が「住所だから」だと、例外に当たるかどうかを判定できない。**

    住所の細かさは手当ての形であって、出さない理由ではない。
    順番を逆にすると、**住所さえ粗ければ出してよい**と読める。

    ここは読む人が実際に見る3枚だけを見る（`index.html` / `about.md` /
    `README.md`）。コードの中のコメントまで語で縛ると、
    語について書けなくなる。
    """

    枚 = (("index.html", "物件そのものの一覧は出しません"),
          ("about.md", "個別の物件を載せません"),
          ("README.md", "1件1行のデータは公開側に出さない"))

    def test_出さないと書いた近くに居住の語がある(self):
        for name, 文 in self.枚:
            with io.open(os.path.join(HERE, name), encoding="utf-8") as f:
                本文 = f.read()
            i = 本文.find(文)
            self.assertNotEqual(i, -1,
                                "%s から『%s』が消えた。"
                                "文を変えたならこの検査も直すこと" % (name, 文))
            窓 = 本文[i:i + 400]
            self.assertIn(
                "住んで", 窓,
                "%s: 個票を出さない理由に『人が住んでいる』が無い。"
                "**住所だからではなく、居住の可能性があるから**"
                "（正本 3.1）:\n%s" % (name, 窓))

    def test_records_が空な理由も居住の語で書いてある(self):
        with io.open(os.path.join(HERE, "README.md"), encoding="utf-8") as f:
            本文 = f.read()
        i = 本文.find("`records` は**いつも空**になる")
        self.assertNotEqual(i, -1, "README から records の説明が消えた")
        窓 = 本文[max(0, i - 400):i]
        self.assertIn("住んで", 窓,
                      "records が空な理由が居住の語で書かれていない:\n" + 窓)
