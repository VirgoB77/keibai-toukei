...
def to_town(addr, town=""):
    ...
    **町丁目には丁目を含める**（正本 4節「丁目は必ず含める」）。ところが
    ...
    s = (addr or "").strip()
    if not town:
        return _banchi_wo_otosu(s)
    # 「大阪市北区中津3-1-1」→「大阪市北区中津3」。頭の市区町村名はそのまま残す。
    #
    # **前から探す。後ろから探していた**（2026-09-19 に直した）。
    # `addr` には建物名が残る。町名が建物名にもう一度出ると、
    # 後ろから探すと**切る位置が地番の後ろに飛ぶ**。
    #
    #     to_town("大阪市北区梅田1-1-1梅田1館", "梅田1")
    #       前から  大阪市北区梅田1          切れている
    #       後ろから 大阪市北区梅田1-1-1梅田1  **地番が丸ごと出る**
    #
    # 丸めたつもりで丸まっていない。**individual の行でここが効く。**
    i = s.find(town)
    if i >= 0:
        切った = s[:i + len(town)]
        # 切った先に番地が残っていないことを見る。町丁目に `数字-数字` は無い
        # （丁目の数字は1つだけ。ハイフンでつながるのは番地から）。
        # 渡された町丁目がこの住所のものでなければ、ここで落ちる
        if not re.search(r"[0-9]-[0-9]", 切った):
            return 切った
    # 見つからない・切っても番地が残る → **きつい側に倒す。**
    # 前は `town` だけを返していたので、**市区町村名が消えていた**
    #     to_town("西宮市上ケ原2番町3-5", "上ケ原二番町") → "上ケ原二番町"
    return _banchi_wo_otosu(s)


# 数字のうしろが町名の続きのときは、まだ番地に入っていない。
# 「甲子園7番町」「北十二条西」など。ここで切ると町名が消える。
# **common/addr.py の `_TOWN_AFTER_NUM` と同じ語。**
# 写しなのは向きのため。5節の関数は姉妹サイトと共有する側で、
# `common/addr.py` はこのサイトのもの。共有する側がこちらを読むと、
# 向きが逆になる。**写したことをここに書いておく。**
_MACHI_NO_TSUZUKI = re.compile(r"^(番町|番丁|条|線|軒|丁目|町|丁)")


def _banchi_wo_otosu(s):
    """番地から後ろを落とす。**最初の番地で切る。**

    前は末尾の数字の並びだけを落としていた。
    番地のうしろに何か付いている住所で、**落ちずにそのまま出ていた。**

        東大阪市吉田6-627-3、627-13   地番が2つ並ぶ（実データに15/106行）
          末尾だけ落とす  東大阪市吉田6-627-3、627-   **地番が出る**
          最初で切る      東大阪市吉田6                切れている

        大阪市北区梅田1-1-1ハイツ101  建物名と部屋番号
          末尾だけ落とす  大阪市北区梅田1-1-1ハイツ   **地番が出る**
          最初で切る      大阪市北区梅田               切れている

    町丁目を渡せないときに通る道なので、**丁目も一緒に落ちる**（きつい側）。
    """
    s = (s or "").strip()
    i = 0
    while True:
        m = re.search(r"[0-9][0-9\-]*", s[i:])
        if not m:
            return s.rstrip("-")
        start, stop = i + m.start(), i + m.end()
        t = _MACHI_NO_TSUZUKI.match(s[stop:])
        if t:
